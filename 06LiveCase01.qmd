...
We will analyze a real-world, recent dataset containing information about the S&P500 stocks. The dataset is located in a 
<a href="https://docs.google.com/spreadsheets/d/11ahk9uWxBkDqrhNm7qYmiTwrlSC53N1zvXYfv7ttOCM/edit#gid=0" target="_blank">Google Sheet</a>

The data is disorganized and challenging to understand. We will review the data and proceed in a step-by-step manner.



### 1) Read the S&P500 data from a Google Sheet into a tibble dataframe.
...
1. We want to understand the different data columns and their data structure.
...
a. The column names described in Table 1a. concern basic information about each stock. 
...
b. The column names described in Table 1b. have Technical information about each stock, including the 52-Week High and Low prices.
...
### 6) Convert columns to factor()

code using base R to change the data type of the "Sector" and "Industry" columns in the sp500 tibble to factor variables:

```{r, warning=FALSE, eval=TRUE, echo=FALSE}
sp500$Sector <- as.factor(sp500$Sector)
sp500$Industry <- as.factor(sp500$Industry)
```
