...
*July 25, 2023 V6 (Work in progress)*
...
### 2. Reading the mtcars dataset as a tibble
...
### 3. Using `dplyr` to explore the `mtcars` tibble

* **`filter()`:**  Recall that this function is used to select subsets of rows in a data frame. It takes logical conditions as inputs and returns rows where the conditions hold true. Suppose we want to filter the mtcars dataset for rows where the mpg is greater than 20.

```{r, warning=FALSE}
filtered_data <- tb %>% filter(mpg > 20)
filtered_data
```

The tibble 'filtered_data' contains only the rows where the miles per gallon (mpg) are greater than 20.

* **`select()`:**: Recall that this function is used to select specific columns. Suppose we want to select 'mpg' and 'hp' columns from mtcars dataset

```{r, warning=FALSE}
selected_data <- tb %>% select(mpg, hp)
selected_data
```

The tibble 'selected_data' will only contain the 'mpg' (miles per gallon) and 'hp' (horsepower) columns from the mtcars dataset.

* **`arrange()`:** Recall that this function is used to reorder rows in a data frame by one or more variables. By default, it arranges rows in ascending order. Suppose we want to select only the mpg and hp columns from the mtcars data and sort it in descending order of mpg
...
* **`mutate()`:** Recall that this function is used to create new variables (columns) or modify existing ones. Suppose we want to create a new column named 'efficiency', defined as the ratio of mpg to hp in the mtcars dataset.
...
* **`summarise()`:**: Recall that this function is used to create summaries of data. It collapses a tibble to a single row. Suppose we want to calculate the mean of 'mpg' in the mtcars dataset
...
To include additional statistical measures such as median, quartiles, minimum, and maximum in your summary data, we can use respective R functions within the `summarise()` function. 
...
summary_data
```
We could convert this back into a standard dataframe and display it.
...
If we wanted to display only two decimal places, we could try
...
**Illustration of %>%**: Using the `mtcars` dataset suppose we want to:
...
### 4. Using `dplyr` to explore the `mtcars` tibble

* **`rename()`:** Remember that this function is helpful in changing column names in our data. For instance, let's modify the name of the mpg column to MPG in the mtcars dataset.

```{r, warning=FALSE}
renamed_data <- mtcars %>% rename(MGP = mpg)
renamed_data
```

The dataframe renamed_data now includes the MPG column, which was previously named mpg.

* **`group_by()`:** This function is key for performing operations within distinct groups of our data. For example, let's group the mtcars dataset by the cyl (number of cylinders) column.
...
* **`slice()`:** Remember that this function is beneficial when we wish to choose rows based on their positions. For example, let's select the first three rows of the mtcars dataset.

```{r, warning=FALSE}
sliced_data <- mtcars %>% slice(1:3)
sliced_data
```

In this sliced_data dataframe, only the first three rows from the mtcars dataset are included.

* **`transmute()`:** Recall that if we desire to create new variables and keep only these variables, we apply the transmute() function. Suppose we want to create a new variable that is the ratio of horsepower (hp) to weight (wt), and keep only this new variable.

```{r, warning=FALSE}
transmuted_data <- mtcars %>% transmute(hp_to_wt = hp/wt) %>% head()
transmuted_data
```

The transmuted_data dataframe now includes the newly created hp_to_wt variable, while the other columns have been removed.

* **`pull()`:**: Recall that this function is employed to remove a single variable from a dataframe as a vector. Let's isolate the mpg (miles per gallon) variable from the mtcars dataset.

```{r, warning=FALSE}
pulled_data <- mtcars %>% pull(mpg)
pulled_data
```

In the pulled_data vector, only the values from the mpg variable are retained.

* **`n_distinct():**: Recall that this function is used to count the distinct values in a column or vector. Let's count the number of distinct values in the cyl (cylinders) column from the mtcars dataset.

```{r, warning=FALSE}
distinct_count <- mtcars %>% summarise(n_distinct_cyl = n_distinct(cyl))
distinct_count
```
...
The chapter has introduced and provided a detailed overview of `tibbles` and the `dplyr` package in the R programming language. 

It started with an introduction to `tibbles`, a data structure in R that is an updated version of data frames with enhanced features for flexible and effective data management. These benefits include more user-friendly printing, reliable subsetting behavior, transparent handling of data types, and support for non-syntactic column names.

Subsequently, the chapter shifted focus to the `dplyr` package, which has been explained as a powerful tool for data manipulation in R. This package offers a cohesive set of functions, often referred to as "verbs", which allow for efficient and straightforward manipulation of data. The key "verbs" in `dplyr`—`filter()`, `select()`, `arrange()`, `mutate()`, and `summarise()`— have been explained and illustrated with examples.

An integral component of the `dplyr` package, the pipe operator `%>%`, has also been discussed in detail. This operator allows for a more readable and understandable chaining of multiple operations in R, leading to cleaner and more straightforward code.

The chapter has given a comprehensive illustration of using `dplyr` on the `mtcars` dataset. This practical demonstration has involved applying `dplyr` functions to a real dataset and explaining the process and results.
