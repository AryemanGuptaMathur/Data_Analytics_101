...
3.  To achieve this, we demonstrate the use of libraries such as `dplyr` and `ggplot2`. Incorporating categorical variables into our analysis elevates the depth of our comprehension of the data. It allows us to shift from a broad, holistic view to a more concentrated inspection of specific data segments. This approach empowers us to produce a more granular and nuanced interpretation of our data.

4.  **Data**: Let us work with the same `mtcars` data from the previous chapter. Suppose we have setup the `mtcars` data in a tibble named `tb`, by running the following code:
